from tool import update_ui, get_conf, trimmed_format_exc, get_max_token, Singleton
import threading
import os
import logging
import numpy as np
...
def input_clipping(inputs, history, max_token_limit):
    ...
    # 去除用户输入的文件路径的那一次对话内容
    # new_history = [history[x] for x in range(1, len(history), 2)]
    print(f'input:{inputs}')
    # for each in history:
    #     print(f"history: {each}")
        ...
        while True:
            ...
            try:
                ...
                if handle_token_exceed:
                    exceeded_cnt += 1
                    # 【选择处理】 尝试计算比例，尽可能多地保留文本
                    from tool import get_reduce_token_percent
                    p_ratio, n_exceed = get_reduce_token_percent(str(token_exceeded_error))
                    MAX_TOKEN = get_max_token(llm_kwargs)
                    EXCEED_ALLO = 512 + 512 * exceeded_cnt
                    inputs, history = input_clipping(inputs, history, max_token_limit=MAX_TOKEN-EXCEED_ALLO)
