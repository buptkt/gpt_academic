本项目改良自GPT-ACADEMIC，但为了便于本地调试使用，以run.py为启动文件，利用原项目相关函数接口，对截断策略等进行了改动，形成了新的py文件，原项目文件没有改动

使用时参考run.py文件内容，在里面设置好待分析项目路径即可，项目日志将会在gpt_log/default_user/shared目录下
